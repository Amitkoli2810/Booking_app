    ...
    const applyFilter=(filter)=>{
        ...
        switch(filter){
            case "Cost: High To Low":
                searchPlaces.map((val)=> val.properties.sort((a,b)=> b.newPrice-a.newPrice))
                setSortedData(searchPlaces)
                break;
            case "Cost: Low To High":
                searchPlaces.map((val)=>val.properties.sort((a,b)=>a.newPrice-b.newPrice))
                setSortedData(searchPlaces)
                break;
        }
    }
        ...
        <View>
            ...
            <ScrollView>
                {sortedData?.filter((item) => item.place === route.params.place).map((item) => item.properties.map((property, index) => (
                    <PropertyCard key={index} rooms={route.params.rooms} children={route.params.children}
                        adults={route.params.adults} selectedDate={route.params.selectedDate} property={property} availableRooms={property.availableRooms} />
                )))}
            </ScrollView>
            <BottomModal
                swipeThreshold={200} onBackdropPress={() => setModalVisible(!modalVisible)} swipeDirection={['up', 'down']} footer={<ModalFooter>
            ...
            >
                <ModalContent style={{ width: '100%', height: 280 }}>
                    <View style={{ flexDirection: 'row' }}>
                        <View style={{ marginVertical: 10, flex: 2, height: 280, borderRightWidth: 2, borderColor: '#E0E0E0' }}>
